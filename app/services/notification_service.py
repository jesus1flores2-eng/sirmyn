        ...
        with app.app_context():
            ...
            for responsable in responsables:
                ...
                if responsable['tipo_mensaje'] == 'completo_con_botones':
                    mensaje = await construir_mensaje_completo(reporte, localidad, calle, app)  # ✅ PASAMOS app
                    ...
                    reply_markup = None  # Sin botones
                
                else:
                    # Mensaje informativo con botón "Ver Detalles" (para otros casos)
                    mensaje = (
                        f"📋 *NUEVO REPORTE*\n\n"
                        f"Folio: #{reporte.id}\n"
                        f"Ubicación: {calle.nombre if calle else 'N/D'} #{reporte.numero}\n"
...
async def construir_mensaje_completo(reporte, localidad, calle, app):
    ...
    if reporte.evidencia:
        upload_folder = app.config.get('UPLOAD_FOLDER', 'uploads')  # ✅ AHORA app está definido
...
# ============================================================
# 4. NOTIFICAR SUPERVISOR REVISIÓN
# ============================================================
async def notificar_supervisor_revision(reporte_id: int, team_id: int):
    ...
    Para agua: supervisor de agua. Para otras áreas: se usa notificar_director_validacion.
    """
    try:
        app = DatabaseManager.get_app()
        with app.app_context():
            ...
            # 1. Obtener reporte y asignación
            reporte = Report.query.get(reporte_id)
            if not reporte:
                logger.error(f"❌ Reporte {reporte_id} no encontrado")
                return False
            ...
            # 2. Buscar supervisor del área (para agua, supervisor con área 'agua')
...
async def notificar_presidente_reporte(reporte_id: int, motivo: str = "nuevo_reporte"):
    ...
    try:
        ...
        with app.app_context():
            ...
            # 3. Construir mensaje según motivo
...
# ============================================================
# 8. NOTIFICAR DIRECTOR DE ÁREA NO-AGUA PARA VALIDACIÓN
# ============================================================
