...
async def manejar_texto_evidencia(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    if "omitir" in texto or "➡️" in texto:
        ...
        await update.message.reply_text("Por favor, selecciona una opción: Envía una foto/video o escribe 'omitir'.")
        return EVIDENCIA

async def manejar_multimedia_evidencia(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    try:
        await update.message.reply_text("📤 Guardando evidencia, por favor espere...", reply_markup=ReplyKeyboardRemove())
        ...
        await update.message.reply_text("❌ Error al procesar el archivo. Intenta con otro o escribe 'omitir'.")
        return EVIDENCIA
