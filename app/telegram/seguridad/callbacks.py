...
async def jefe_seguridad_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    try: await query.answer()
    except: pass
    callback_data = query.data
    if not callback_data.startswith('seguridad_') or not callback_data.startswith(('seguridad_validar_', 'seguridad_rechazar_')): return
    ...
    with app.app_context():
        usuario = User.query.filter_by(telegram_id=str(query.from_user.id), area='seguridad', is_active=True).first()
        ...
        if accion == 'validar':
            ...
            asignacion.status_id = estado_finalizado.id; asignacion.observaciones = f"Validado por Jefe de Seguridad {usuario.nombre}"; db.session.commit()
            await query.message.reply_text(f"✅ *VALIDADO POR JEFE DE SEGURIDAD*\n📅 {datetime.now().strftime('%d/%m/%Y %H:%M')}\n👷 Cuadrilla: {cuadrilla.nombre if cuadrilla else 'N/D'}\n🏷️ Estado: Finalizado ✓", parse_mode=ParseMode.MARKDOWN)
            from app.services.notification_service import notificar_usuario_reporte_finalizado
            await notificar_usuario_reporte_finalizado(reporte, asignacion, "Jefe de Seguridad")
            await query.answer("✅ Reparación validada", show_alert=False)
        elif accion == 'rechazar':
            user_data[query.from_user.id] = {'modo_esperando_motivo_rechazo_seguridad': True, 'reporte_id': reporte_id, 'cuadrilla_id': asignacion.team_id, 'cuadrilla_nombre': cuadrilla.nombre if cuadrilla else 'Cuadrilla desconocida'}
...
async def manejar_motivo_rechazo_jefe_seguridad(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with app.app_context():
            ...
            jefe = User.query.filter_by(telegram_id=str(user_id)).first(); nombre_jefe = jefe.nombre if jefe else "Jefe de Seguridad"
            ...
            asignacion = Assignment.query.filter_by(report_id=reporte_id).order_by(Assignment.timestamp.desc()).first()
            if asignacion: asignacion.status_id = estado_en_proceso.id; asignacion.observaciones = f"Rechazado por Jefe de Seguridad. Motivo: {motivo}"; db.session.commit()
