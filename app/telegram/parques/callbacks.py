...
async def jefe_parques_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not callback_data.startswith('parques_') or not callback_data.startswith(('parques_validar_', 'parques_rechazar_')):
        return
    ...
    with app.app_context():
        usuario = User.query.filter_by(telegram_id=str(query.from_user.id), area='parques', is_active=True).first()
        ...
        if accion == 'validar':
            ...
            asignacion.observaciones = f"Validado por Jefe de Parques {usuario.nombre}"
            db.session.commit()
...
async def manejar_motivo_rechazo_jefe_parques(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with app.app_context():
            ...
            nombre_jefe = jefe.nombre if jefe else "Jefe de Parques"
            ...
            if asignacion:
                ...
                asignacion.observaciones = f"Rechazado por Jefe de Parques. Motivo: {motivo}"
                db.session.commit()
            bot = context.bot
